...
int Goban::legal_moves(int moves[]) const
{
  ...
  for (int i = 0; i < empty_points.length(); i++) {
    ...
    if (is_legal(point, side) && !is_true_eye(point, side, 0)) {
      moves[nlegal++] = point;
    }
  }
  moves[nlegal++] = PASS;
...
int Goban::relative_point(int point, int horizon, int vertical) const
{
  int point_h = point%size;
  int point_v = point/size;
  ...
  point_v = (point_v + vertical)%13;
  point_h = (point_h + horizon)%13;
  if (point_h == 0)
    point_h = 13;

  return point_v*size + point_h;
}
