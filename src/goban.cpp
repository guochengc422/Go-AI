...
void Goban::cpy_goban(Goban *origin){
	reset();
	set_fixed_handicap(handicap);
	for (int i = 0; i < origin->game_history.length(); i++) {
		if (origin->game_history[i]) {
			drop_stone(origin->game_history[i], side);
			side = !side;
		}
		else {
			side = !side;
	...
	this->ko_point = origin->ko_point;
	this->size2 = origin->size2;
	this->size = origin->size;
	this->komi = origin->komi;
	this->handicap = origin->handicap;

	for (int i = 0; i <= MAXSIZE2; i++){
		this->points[i] = origin->points[i];
		this->groups[i] = origin->groups[i];
		this->empty_points.points[i] = origin->empty_points.points[i];
	}
	this->stones_on_board[0] = origin->stones_on_board[0];
	this->stones_on_board[1] = origin->stones_on_board[1];
	this->last_atari[0] = origin->last_atari[0];  //One for black, one for white.
	this->last_atari[1] = origin->last_atari[1];  //One for black, one for white.
	this->last_point = origin->last_point;
	this->last_point2 = origin->last_point2;*/
